...
using static Heirloom.Sound.Desktop.NativeApi;

namespace Heirloom.Sound.Desktop
{
    internal sealed unsafe class DesktopAudioImplementation : AudioImplementation
    {
        private readonly void* _device;
        private readonly DataProcessCallback _dataProc;

        internal DesktopAudioImplementation(int sampleRate = AudioBackend.SampleRate, bool enableAudioCapture = false)
        {
        ...
        private void DataProcessCallback(void* pDevice, void* pOutput, void* pInput, uint frameCount)
        {
            ...
            var sampleCount = (int) frameCount * AudioBackend.Channels;

            // Process audio (read, decode, etc)
            if (pInputSamples != null) { AudioBackend.GatherInputSamples(new Span<short>(pInputSamples, sampleCount)); }
            if (pOutputSamples != null) { AudioBackend.GatherOutputSamples(new Span<short>(pOutputSamples, sampleCount)); }
        }
