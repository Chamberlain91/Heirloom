...
{
    internal static unsafe partial class GLES
    {
        ...
        public static void LoadFunctions(GetProcAddress getProcAddress)
        {
            ...
            foreach (var field in typeof(GLES).GetFields(BindingFlags.NonPublic | BindingFlags.Static))
            {
                if (field.Name.StartsWith("gl"))
                {
                    ...
                    else
                    {
                        ...
                        if (procAddr == IntPtr.Zero)
                        {
                            ...
                            Log.Error($"Unable to load OpenGL function '{field.Name}', address returned null.");
                        }
                        else
                        {
        ...
        public static ClientWaitSyncResult ClientWaitSync(ulong sync, ClientWaitSyncFlags flags)
        {
            ...
            CheckError(nameof(ClientWaitSync));
            return x;
        }
