...
namespace Heirloom.Runtime
{
    internal class AsepriteFile : IDisposable
    {
        ...
        public AsepriteFile(Stream stream)
        {
            ...
            Log.Debug($"Loading Sprite");
            Log.Debug($"  Dimensions: {Width} x {Height} ({_colorDepth} bit)");
            Log.Debug($"  Frames: {_frameCount}");
            Log.Debug($"  Colors: {_numberOfColors}");
            Log.Debug($"  Pixel Aspect: {_pixelWidth}:{_pixelHeight}");
        ...
        private Pixel[] ReadPaletteChunk()
        {
            ...
            Log.Debug($"  Reading New Palette: {size} from {from} to {to}");
            ...
            for (var i = from; i <= to; i++)
            {
                ...
                {
                    var name = ReadString();
                    Log.Debug($"    Color {i}: {palette[i]} ({name})");
                }
                else
                {
                    Log.Debug($"    Color {i}: {palette[i]}");
                }
            }
        ...
        private byte[] ReadCompressedBytes(int n)
        {
            ...
            using (var ms = new MemoryStream(ReadBytes(n - 6))) // ingore 2 for header, 4 for checksum
        ...
        internal SpriteCollection ConstructSpriteCollection()
        {
            // Merge sprite frame images into atlas
            Image.CreateAtlas(Frames.Select(f => f.Image));

            // Create sprite
            var collection = new SpriteCollection();

            // assume each tag is a named animation
            foreach (var tag in Tags)
            {
                ...
                for (var i = tag.From; i <= tag.To; i++)
                {
                    ...
                    // Append frame to sprite
                    // todo: animation direction...?
                    sprite.Add(frame.Image, frame.Duration / 1000F);
                }
            }

            // If we have a slice on the first frame, assume its pivot
            // TODO: Better pivot design? Per image? How does aseprite encode this?
            if (Frames[0].Slices.Count > 0)
            {
                var slice = Frames[0].Slices[0];
                foreach (var sprite in collection.Sprites)
                {
                    sprite.Origin = (slice.PivotX, slice.PivotY);
                }
            }
