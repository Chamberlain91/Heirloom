...
{
    public sealed class Image2 : IImageSource
    {
        ...
        /// </summary>
        internal static Image2 White = CreateColor(1, 1, Color.White);
        ...
        public void CopyTo(in IntRectangle region, Image2 target, in IntVector targetOffset)
        {
        ...
        public static Image2 CreateCheckerboardPattern(int width, int height, Color color, int cellSize = 16)
        {
            var im = new Image2(width, height);
        ...
        public static Image2 CreateGridPattern(int width, int height, Color color, int cellSize, int borderWidth = 1)
        {
            var im = new Image2(width, height);
        ...
        public static Image2 CreateColor(int width, int height, Color color)
        {
            var im = new Image2(width, height);
        ...
        public static Image2 CreateNoise(int width, int height, float scale = 1, int octaves = 4, float persistence = 0.5F)
        {
        ...
        /// <param name="persistence">How persistent each noise layer is.</param>
        /// <returns>A noisy image with noise generated on all four components.</returns>
        public static Image2 CreateNoise(int width, int height, INoise2D noise, float scale = 1, int octaves = 4, float persistence = 0.5F)
        {
        ...
        public static unsafe void Copy(Image2 source, in IntRectangle sourceRegion,
                                       Image2 target, in IntVector targetOffset)
        {
        ...
        public static unsafe void WriteAsPng(Image2 image, Stream stream)
        {
            lock (image.Pixels)
            {
                _writeStream = stream;
        ...
        public static unsafe void WriteAsJpg(Image2 image, Stream stream, int quality = 85)
        {
            lock (image.Pixels)
            {
        ...
        public static unsafe Image2 Load(Stream stream)
        {
            return Load(stream.ReadAllBytes());
        }

        public static unsafe Image2 Load(byte[] file)
        {
            fixed (byte* filePtr = file)
            {
                ...
                var data = new Image2(width, height);
