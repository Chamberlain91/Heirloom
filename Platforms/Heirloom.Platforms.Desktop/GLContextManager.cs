...
namespace Heirloom.Platforms.Desktop
{
    ...
    internal class GLRenderContext : OpenGLRenderContext
    {
        ...
        public override void SwapBuffers()
        {
            ...
            /* 
             * Conceptually, I should be able to use non-blocking Invoke for every case, as
             * logically speaking it is a queue. I should be able to submit to the queue without
             * waiting and still have expected ordering. When using non-blocking Invoke in a single
             * window case an unexpected occasional freeze seems to occur and I am not sure why. The
             * need for the non-blocking invoke is to avoid waiting for each windows swap to finish
            ...
            Invoke(() => Glfw.SwapBuffers(Window.Native), Window.Windows.Count == 1);
        }
    }
}
